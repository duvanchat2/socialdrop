...
export function WorkspaceSwitcher() {
  ...
  return (
    <div className="relative px-3 pb-3">
      ...
      {open && (
        <div className="absolute left-3 right-3 mt-1 z-40 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-lg overflow-hidden">
          {workspaces.map((w) => (
            <button
              key={w.id}
              onClick={() => select(w.id)}
