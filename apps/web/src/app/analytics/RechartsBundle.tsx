    ...
    <div className="space-y-6">
      ...
      <ChartCard title="Seguidores en línea — Mejor momento para publicar">
        ...
        <div className="flex items-center gap-3 mt-3 text-xs text-gray-400">
          ...
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-gray-100 dark:bg-gray-700 inline-block" />Inactivo</span>
        </div>
      </ChartCard>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
        ...
        <ChartCard title="Distribución por tipo">
          {total > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie
                  data={postTypeData.map(d => ({ name: d.type, value: d.value }))}
      ...
      <ChartCard title="Historial de publicaciones">
        <MiniChart
          data={followerData.map(d => ({ date: d.date, value: Math.floor(Math.random() * 4) }))}
          color="#6366f1"
          height={160}
        />
      </ChartCard>
